...
#ifdef USE_RUNTIME_D3D_SHADER_COMPILER
#error "Messed up the build setup somehow..?"
#else
#define USE_RUNTIME_D3D_SHADER_COMPILER 0
#endif

#if USE_RUNTIME_D3D_SHADER_COMPILER
...
#if USE_RUNTIME_D3D_SHADER_COMPILER
static constexpr const char* vertexShaderSourceCode =
	...
	static const char* pixelShaderSourceCode =
		"struct PS_INPUT\
            {\
            float4 pos : SV_POSITION;\
            float4 col : COLOR0;\
    ...
    // Create the vertex shader
    {
        ...
        if (FAILED(D3DCompile(vertexShaderSourceCode, strlen(vertexShaderSourceCode), NULL, NULL, NULL, "main", "vs_4_0", 0, 0, &vertexShaderBlob, NULL)))
...
#else
		if (bd->pd3dDevice->CreateVertexShader(vertexShaderBytecode, sizeof(vertexShaderBytecode), NULL, &bd->pVertexShader) != S_OK)
			return false;
#endif
        ...
        vertexShaderBlob->Release();
#else
		if (bd->pd3dDevice->CreateInputLayout(local_layout, 3, vertexShaderBytecode, sizeof(vertexShaderBytecode), &bd->pInputLayout) != S_OK)
			return false;
#endif
    ...
    // Create the pixel shader
    {
        ...
        if (FAILED(D3DCompile(pixelShaderSourceCode, strlen(pixelShaderSourceCode), NULL, NULL, NULL, "main", "ps_4_0", 0, 0, &pixelShaderBlob, NULL)))
            return false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
        ...
        pixelShaderBlob->Release();
#else
		if (bd->pd3dDevice->CreatePixelShader(pixelShaderBytecode, sizeof(pixelShaderBytecode), NULL, &bd->pPixelShader) != S_OK)
			return false;
#endif
    }
...
static void ImGui_ImplDX11_CreateWindow(ImGuiViewport* viewport)
{
	...
	bd->pFactory->MakeWindowAssociation(hwnd, DXGI_MWA_NO_ALT_ENTER);
